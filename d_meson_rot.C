void d_meson_rot(TString filename = "results_all.root", int rebin = 10) {
	...
	TH1D *h_inv_mass_rot = (TH1D*)(_file0->Get("h_inv_mass_ROT"));
	//h_inv_mass_rot->Sumw2();
	//double int_rot = h_inv_mass_rot->Integral(1100,1250);
	//double int_rot = h_inv_mass_rot->Integral(1100,1550);
	//double int_rot = h_inv_mass_rot->Integral(1200,1700);
	double int_rot = h_inv_mass_rot->Integral(1500,1700);
	//double int_rot = h_inv_mass_rot->Integral(1500,1800);
	//double int_rot = 1;
	h_inv_mass_rot->Rebin(rebin);
	...
	//double int_us = h_inv_mass_US->Integral(1500,1800) ;
	//double int_us = 1;
	...
	*/
	...
	c2->cd(1);
	//h_inv_mass_rot->Scale(h_inv_mass_US->Integral(1100,1250)/h_inv_mass_rot->Integral(1100,1250));
	cout << "integral us: " << int_us << " rot: " << int_rot << endl;
	h_inv_mass_rot->Scale(int_us/int_rot);
	h_inv_mass_rot->SetLineColor(kBlue);
	h_inv_mass_rot->SetTitle("");
	h_inv_mass_rot->GetXaxis()->SetRangeUser(0.5,2.2);
	//h_inv_mass_rot->GetYaxis()->SetNdivisions(208);
	h_inv_mass_rot->GetYaxis()->SetTitleOffset(1.5);
	h_inv_mass_rot->GetYaxis()->SetTitle("Raw Counts");
	h_inv_mass_rot->GetXaxis()->SetTitle("Mass_{K#pi} (GeV/c^{2})");
	h_inv_mass_rot->SetLineColor(kRed);
	h_inv_mass_rot->DrawCopy();
	//h_inv_mass_rot->Draw("sames");
	h_inv_mass_US->DrawCopy("sames");
	//h_inv_mass_rot->DrawCopy("sames");
	...
	leg->AddEntry(h_inv_mass_rot,"Rotated kaon (Rot)","P");
	leg->Draw();


	cout << "bin US ROT " << endl;
	for(int ib=180; ib<200; ib++) {
		cout << ib << " " << h_inv_mass_US->GetBinError(ib) << " " << h_inv_mass_ROT->GetBinError(ib) << endl;
	}

	/*
	c2->cd(2);
	h_inv_mass_rot->SetLineColor(kBlue);
	h_inv_mass_rot->GetXaxis()->SetRangeUser(1.7,2.0);
	h_inv_mass_rot->DrawCopy();
	//h_inv_mass_rot->Draw("sames");
	...
	*/
	...
	//c2->cd(3);
	c2->cd(2);
	h_inv_mass_US->Add(h_inv_mass_rot, -1);
	h_inv_mass_US->GetYaxis()->SetTitleOffset(1.5);
	h_inv_mass_US->DrawCopy();
	tx1.DrawLatex(0.37,0.84,"SE-Rot");
	...
	*/
	...
	tx1.DrawLatex(0.17,0.84,"SE-Rot");
	...
	output_file.ReplaceAll(".root","_d_meson_rot.pdf");
